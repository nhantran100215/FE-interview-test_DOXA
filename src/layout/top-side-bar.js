...
const TopSideBar = () => {
  return (
    ...
    {/* title header */}
    <div className="relative flex items-center justify-center px-7 w-auto -translate-y-3">
    <img
          src={logoRGift}
          alt="iconRedditText"
          className="mr-5 h-[72px] w-[72px] border-4 rounded-full border-white bg-white "
        />
    <span className="text-3xl font-bold overflow-hidden leading-8 text-ellipsis">.gifs - funny, animated gifs for your viewing pleasure</span>
    <button className="px-8 py-1 ml-5 bg-indigo-500 rounded-2xl font-bold text-white hover:bg-indigo-400">join</button>
    </div>
    </div>
  );
};
